...
async function checkFelipeReAudit() {
  // Buscar profile do Felipe pelo ID
  ...
  if (profileError || !profile) {
    console.log('❌ Perfil do Felipe não encontrado');
    process.exit(1);
  }

  console.log('👤 Perfil encontrado:', profile.full_name, '@' + profile.username);
  console.log('');

  // Buscar auditorias do Felipe
  ...
  if (auditError || !audits || audits.length === 0) {
    console.log('❌ Nenhuma auditoria encontrada para o Felipe');
    process.exit(1);
  }
  ...
  if (reAudits.length === 0) {
    ...
    console.log('   1. Entre na auditoria do Felipe');
