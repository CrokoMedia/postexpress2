...
export default function SlidesPage() {
  ...
  const handleDeleteCarousel = async (carouselIndex: number) => {
    ...
    try {
      const res = await fetch(`/api/content/${id}/slides?carouselIndex=${carouselIndex}`, {
        method: 'DELETE',
      })
      if (!res.ok) {
        const data = await res.json()
  ...
  const isV2 = slides?.template === 'v2'
  const totalSlides = slides?.summary?.totalSlides || 0
  const totalCarousels = slides?.summary?.totalCarousels || 0
      ...
      <div>
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <PageHeader
            ...
            description={
              slides
                ? `${totalCarousels} carrossel${totalCarousels !== 1 ? 'is' : ''} · ${totalSlides} slide${totalSlides !== 1 ? 's' : ''}`
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-neutral-500">
              Gerado em {new Date(slides.generated_at).toLocaleString('pt-BR')}
            </span>
          </div>
