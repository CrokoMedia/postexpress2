...
/**
 * 🔄 CAROUSEL TO FIGMA CONVERTER
 *
 * Converte carrosséis do Content Creation Squad para Stories do Figma
 * Transforma formato de 10 slides em 7 Stories otimizados
 *
...
function convertToStories(carousel) {
  ...
  // Slide 1 (Hook) → Story 1
  // Slide 2 (Problema) → Story 2
  // Slides 3-7 → Stories 3-5 (principais pontos)
  // Slide 8-9 → Story 6 (consolidação)
  // Slide 10 (CTA) → Story 7
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h1 { text-align: center; margin-bottom: 20px; color: #FFD23F; }
...
<body>
  <div class="container">
    <h1>🎨 Preview: ${carouselTitle}</h1>
    <div class="subtitle">${stories.length} Stories • Formato Instagram</div>
    <div class="stories-grid">
      ${stories.map(story => `
        <div class="story ${story.background}" style="position: relative;">
...
async function main() {
  console.log('🔄 CAROUSEL TO FIGMA CONVERTER\n');
  ...
  // Por padrão, converter o primeiro
  const selectedFile = carouselFiles[0];
  const selectedTitle = titles[0] || 'Carrossel 1';
  ...
  const pluginPath = path.join(CONFIG.OUTPUT_DIR, 'figma-plugin-custom.js');
  ...
  const htmlPath = path.join(CONFIG.OUTPUT_DIR, 'preview-custom.html');
  await fs.writeFile(htmlPath, htmlPreview, 'utf8');
  console.log(`✅ Preview HTML: ${htmlPath}`);

  // Gerar JSON
  const jsonPath = path.join(CONFIG.OUTPUT_DIR, 'stories-custom.json');
  ...
  console.log('\n' + '='.repeat(60));
  console.log('\n📖 PRÓXIMOS PASSOS:\n');
  console.log('1️⃣  Ver preview no navegador:');
  console.log(`   open ${htmlPath}\n`);
  console.log('2️⃣  Copiar plugin para Figma Desktop:');
  console.log(`   - Abrir: ${pluginPath}`);
  ...
  console.log('   - Aguardar ~30s');
  console.log(`   - ${stories.length} Stories criados!\n`);
