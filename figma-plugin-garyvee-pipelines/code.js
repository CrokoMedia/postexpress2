...
function createStep(step, x, y, width = 280) {
...
function createArrow(startX, startY, endX, endY, color) {
...
function createProsCons(pros, contras, x, y) {
...
function createListFrame(title, items, color) {
    const frame = figma.createFrame();
    frame.name = title;
...
function createPipeline(option_1) {
    return __awaiter(this, arguments, void 0, function* (option, startX = 0, startY = 0) {
    ...
    if (msg.type === 'create-pipeline') {
        try {
            if (msg.option === 'all') {
                ...
                const frames = [];
                for (let i = 1; i <= 4; i++) {
                    const frame = yield createPipeline(i, (i - 1) * 700, 0);
                    frames.push(frame);
                ...
                const frame = yield createPipeline(msg.option);
                figma.currentPage.appendChild(frame);
